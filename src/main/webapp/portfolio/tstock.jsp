        ...
        <script>
            $(document).ready(function () {
                $("#myTable").on("click", "tr", function () {
                    ...
                    //console.log(id);
                    $.get("/SpringMVC_1128_JPA/mvc/portfolio/tstock/" + id, function (data, status) {
                ...
                $("#add").on("click", function () {
                    ...
                    $.ajax({
                        url: "/SpringMVC_1128_JPA/mvc/portfolio/tstock/",
                        type: "POST",
                ...
                $("#del").on("click", function () {
                    ...
                    $.ajax({
                        url: "/SpringMVC_1128_JPA/mvc/portfolio/tstock/" + id,
                        type: "DELETE",
                        async: false, //Cross-domain requests and dataType: "jsonp" requests do not support synchronous operation
                        cache: false, //This will force requested pages not to be cached by the browser  
                        processData: false, //To avoid making query String instead of JSON
            ...
            function table_list() {
                $.get("/SpringMVC_1128_JPA/mvc/portfolio/tstock/", function (datas, status) {
                    console.log("Datas: " + datas);
                    $("#myTable tbody > tr").remove();
                    $.each(datas, function (i, item) {
                        var html = '<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td>\n\
                                    <td align="right">{4}</td><td align="right">{5}</td>\n\
                                    <td align="right">{6}</td><td align="right">{7}</td>\n\
                                    <td>{8}</td><td align="right">{9}</td></tr>';
